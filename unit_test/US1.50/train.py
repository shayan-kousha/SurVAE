...
def flow(C=3, H=32, W=32, num_flow_steps=32, hidden=256,layer=2):
...
def sampling(params,rng, batch_y,num_samples=4):
    base_images = jnp.transpose(batch_y[:num_samples],(0,2,3,1))
    batch_y = (batch_y + random.uniform(rng,batch_y.shape))/256 - 0.5
    batch_y = jnp.concatenate((batch_y,jnp.ones(batch_y.shape) * (-2.0)), axis=1)
    ...
    return generate_images, base_images
...
def eval(params, dataloader, rng, sample=False):
    ...
    if sample:
        print("===== Sampling ========")
        generate_images, base_images = sampling(params=params,rng=rng,batch_y=y,num_samples=FLAGS.num_samples)
    log_prob = jnp.array(log_prob).mean()
    log_prob /= float(np.log(2.)*3*FLAGS.input_res*FLAGS.input_res)
    # ipdb.set_trace()

    return -log_prob, generate_images, base_images
...
def main():
    ...
    test_loss, samples, base_samples = eval(optimizer.target, test_loader, eval_rng, sample=True)
    ...
    if samples != None:
        ...
        survae.save_image(base_samples, FLAGS.ckptdir+f'/basesample_{start_epoch-1}.png', nrow=8)
