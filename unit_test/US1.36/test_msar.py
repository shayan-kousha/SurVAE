...
test_loader = torch.utils.data.DataLoader(test_ds, batch_size=100, shuffle=True,drop_last=True)
...
def flow2(num_flow_steps=2,C=3, H=32,W=32, hidden=20,layer=2):
    bijections = []
    _H = H
    # layer = int(np.log2(_H))
    for i in range(layer):
        ...
        if i < layer - 1 :
            C //= 2
            _base_dist = survae.AutoregressiveConvLSTM._setup(base_dist=survae.Normal,features=2,kernel_size=(3,3),latent_size=(C,H,W))
            bijections += [survae.Split._setup(survae.Flow._setup(survae.StandardNormal,[],(C,H,W)),C, dim=1)]
    _base_dist = survae.AutoregressiveConvLSTM._setup(base_dist=survae.Normal,features=2,kernel_size=(3,3),latent_size=(C,H,W))
