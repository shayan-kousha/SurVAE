...
class MNIST:
    ...
    def __init__(self, logit=False, dequantize=True):
        ...
        trn, val, tst = pickle.load(f, encoding='bytes')
        f.close()

        rng = np.random.RandomState(42)
        self.trn = self.Data(trn, logit, dequantize, rng)
        print('average: {}, max: {}, min: {}'.format(self.trn.x.mean(), self.trn.x.max(), self.trn.x.min()))
    ...
    def show_images(self, split):
        ...
        # display images
        disp_imdata(data_split.x, self.image_size, [1, 2])
